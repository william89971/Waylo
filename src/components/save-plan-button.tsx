...
export function SavePlanButton({ strategy }: { strategy: RouteStrategy }) {
  ...
  return <div><button className="production-button primary" onClick={() => void save()} disabled={working}>{working ? "Saving plan…" : "Save this plan"}</button>{error ? <p className="production-error" role="alert">{error}</p> : null}</div>;
}
