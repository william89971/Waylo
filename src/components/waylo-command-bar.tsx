...
export function WayloCommandBar({ showFallback = true }: { showFallback?: boolean }) {
  ...
  return (
    <section className="command-shell navigator-command" aria-label="Waylo Live Navigator command">
      ...
      <div className="command-meta"><span>GPT can normalize the request. It cannot edit or validate the route.</span>{workspace.mode === "live" ? <label>Interpreter<select className="compact-select" aria-label="Command interpreter" value={mode} onChange={(event) => setMode(event.target.value as "seeded" | "live")}><option value="seeded">Recorded demo</option><option value="live">Live GPT-5.6</option></select></label> : <span className="seeded-disclosure">Seeded replay · no OpenAI request</span>}</div>
