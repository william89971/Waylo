...
export const PlanChangeSchema = z.discriminatedUnion("type", [
  z.object({
    ...
    namedTerm: z.string().regex(/^(Fall|Spring|Summer) \d{4}$/).optional(),
  }),
  ...
  z.object({ id: z.string(), type: z.literal("set_summer_enrollment"), enabled: z.boolean(), courseLimit: z.number().int().min(0).max(3).optional() }),
  ...
  z.object({ id: z.string(), type: z.literal("set_transfer_target"), term: z.string().regex(/^(Fall|Spring|Summer) \d{4}$/), policy: TransferTargetPolicySchema.default("preferred") }),
